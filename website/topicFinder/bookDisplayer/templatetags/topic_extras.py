...
from bookDisplayer.models import TocBookTopic, BookTopicInfo , TocFreq , BookInfo , ClassInfo, JobCon
import ast 
import operator
import random
...
@register.filter(name = "load_jobs_con")
def load_jobs_con(value,job):
	filtered_out = job.filter(Element1=value.dept_course)
...
@register.filter(name = "load_jobs_des")
def load_jobs_des(value,job):
	filtered_out = job.filter(Element1=value.dept_course)
...
@register.filter(name = "find_job_id")
def find_job_id(value,job):
	filtered_out = job.filter(title=value.Element2)
...
def load_freq(value , freq):
	...
	if(filtered_out):
		...
		if len(freq_dict) == 0:
			return 'Not found'
		return sorted(list(freq_dict.items()) , key = operator.itemgetter(1) , reverse = True)
